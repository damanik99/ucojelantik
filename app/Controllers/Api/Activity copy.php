...
class Activity extends ResourceController
{
	...
	public function postStatus()
	{
		...
		$code = $this->customModel->setCounterNumber('timeline', 'code', 'TML');

		if($picture != "")
		{
			/** server **/
			$url_segment = explode("/", APPPATH);
			echo var_dump($url_segment);exit;
			$patd = $url_segment[0]."/".$url_segment[1]."/".$url_segment[2]."/".$url_segment[3].base_url();
			
			/** localhost **/
			// $url_segment = explode("\\", Yii::app()->basePath);
			// $patd = $url_segment[0]."/".$url_segment[1]."/".$url_segment[2].base_url(); 

			$uploads_dir = '../script_apps/public/upload/activity/';
			$ifp = fopen($uploads_dir.$code.".jpg", "wb"); 
			fwrite($ifp, base64_decode($picture)); 
			fclose($ifp); 
			
			if(is_readable($patd."/".$uploads_dir.$code.".jpg") == true)
				$picture = "http://www.surfgold.co.id".base_url()."/".$uploads_dir.$code.".jpg";
			else
	...
	public function getTimeline()
	{
		...
		`code`, `datetime`, latitude, longitude, location, description, photo, a.created_date, a.modified_date 
				FROM timeline a 
				JOIN users b ON a.users_id = b.users_id 
				WHERE program_id ="'.$check['program_id'].'" AND';
		
		if (count($arr_users) > 0)
			$sql .= ' a.users_id = '.$arr_users.' AND';

		if ($search_by == "username")
			$sql .= ' b.username LIKE "%'.$search_value.'%"';
		
		if($search_by == "type")
			$sql .= ' a.type ="'.$search_value;

		$sql .= ' a.type NOT IN ("Announcement")';

		$type = array();
		if($program->module_activity == 1)
			$type[] = "Share Status";
			
		if($program->module_attendance == 1)	
			$type[] = "Checked in";
			
		if($program->module_selling == 1)
			$type[] = "Selling";
			
		if($program->module_distribution == 1)
			$type[] = "Distribution";
		...
		$timelineCount = $timeline_query->getRow();
		
		$timeline = new timelineModel();

		$page_timeline = [
			'count' => $timeline->paginate(10),
			'pager' => $timeline->pager,
			'curentPage' => $timeline->pager->getCurrentPage(),
			// 'curentPage' => $obj->page-1,

		];
		// echo var_dump($page_timeline);exit;
		if ($page_timeline['curentPage'] == 0) 
		{
			...
			foreach ($announs as $announ) 
			{
				...
				echo var_dump($arr_timeline);exit;
			}
		...
		foreach ($dataProvider as $timelines)
		{
			...
			$page_comment = $query_comment_count->getRow();

			$comment = new timelinecommentModel();

			$page_comment_count = [
				'count' => $comment->paginate($page_comment->count_comment),
				'pager' => $comment->pager,
				'curentPage' => $comment->pager->getCurrentPage(),
			...
			$arr_timeline[] = array(
				...
				'total_comment' => $page_comment->count_comment,
				'comment' => $arr_comment
			);
		...
		$arrData = array(
			'total_page' => intval(round($timelineCount->count / 10)),
