...
class Distribution extends ResourceController
{
    ...
    public function postDistributions()
    {
		...
		foreach($obj as $var=>$value) 
		{
			...
			if($var == "picture_before")
				$pic_bef = $value;
				
			if($var == "picture_after")
				$pic_aft = $value;
		}
		...
		try
		{
			if($users_organization_id) 
			{
				...
				{
					$message = "";
					foreach($arrs as $valuex)
					{
						$checkId = $this->checkId($valuex->outbound_id, "outbound");
						$outbound = $this->db->table('outbound')->select('*')->where('outbound_id', $checkId["id"])->get()->getRow();
						
						if($valuex->quantity > ($outbound->received_quantity - $outbound->distribution_quantity))
							$message .= $outbound->programItem->item->name." Over Quantity. ";
						
						if($pic_bef == "")
							$message .= $outbound->programItem->item->name."Pic Before Empty. ";
							
						if($pic_bef == "")
							$message .= $outbound->programItem->item->name."Pic After Empty. ";
					}
					...
					if($message == "")
					{
						foreach($arrs as $valuex)
						{
							...
							if($valuex->quantity <= ($outbound->received_quantity - $outbound->distribution_quantity))
							{
								...
								$model->outbound_id = $outbound->outbound_id;
								$model->users_organization_id = $users_organization_id;
								$model->program_item_id = $outbound->program_item_id;
								$model->quantity = $valuex->quantity;
								$model->display = $valuex->display;
								$model->remarks = $valuex->remarks;
	
								$tick = round(microtime(true) * 1000);
								$name = $model->outbound_id.$tick;
								...
								{
									// $outbound->distribution_quantity += $model->quantity;
									$outbounds = new OutboundModel();
									$outbounds->update($model->outbound_id, [
										"distribution_quantity" => $outbound->distribution_quantity += $model->quantity,
									]);
									...
									$timeline = [
										...
										'type' => "Distribution",
										'code' => $timeline_code,
										'datetime' => $model->distribution_date,
										'name' => "",
										'location' => $organization->name,
										'description' => "Distribution ".$model->quantity." Unit ".$item->name,
										'photo' => $urlimgAfr,
										'created_date' => date("Y-m-d H:i:s"),
										'modified_date' => date("Y-m-d H:i:s"),
										'created_by' => $check["users_id"],
									...
									$this->db->table('timeline')->insert($timeline);
								}
								else
									$message = "Unkown Error";
							}
							else
								$message = "Over Quantity Distribution";
						}
					}
					else
						$this->_sendResponse(304, $message, '');
					...
					$this->_sendResponse(401, "Item Name can't empty", '');
			}
			else
				$this->_sendResponse(401, "Channel Name can't empty", '');
		}
		catch (Exception $ex) 
		{  
