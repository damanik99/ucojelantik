...
class ECSVExportModel extends Model
{
    /**
    ...
    public function _writeRow($row)
    {
        ...
        if($this->stripNewLines) {    
            echo var_dump(array_walk($row));exit;        
            array_walk($row, array('ECSVExportModel','lambdaFail'));
        }
        
        array_walk($row, array('ECSVExportModel','stripSlashes'));
